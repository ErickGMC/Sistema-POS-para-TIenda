...
async function checkAnalytics() {
  ...
  try {
    const snap1 = await getDocs(query(collection(db, 'analytics'), limit(5)));
    console.log(`Found ${snap1.size} docs in analytics`);
    snap1.docs.forEach(d => console.log(d.id, d.data()));
  } catch (e) {
