...
export default function GestionUsuarios() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!(await useUIStore.getState().showConfirm(editMode ? '¿Estás seguro de que deseas actualizar este usuario?' : '¿Estás seguro de que deseas agregar este nuevo usuario?', 'Guardar Usuario'))) {
      return;
    }
  ...
  const eliminarUsuario = async (id: string) => {
    if (!(await useUIStore.getState().showConfirm('¿Seguro que deseas eliminar a este usuario?', 'Eliminar Usuario'))) return;
  ...
  return (
    <div className="h-full w-full bg-slate-50 p-4 flex gap-4 overflow-hidden">
      
      {/* Lista de Usuarios */}
      <div className="flex-1 bg-white border border-slate-200 rounded-2xl p-5 flex flex-col shadow-2xl">
        <div className="flex items-center gap-3 mb-6">
          <div className="p-3 bg-blue-500/20 text-blue-600 rounded-xl">
            <Users size={24} />
          </div>
          <div>
            <h2 className="text-xl font-bold text-slate-900 tracking-tight">Colaboradores</h2>
            <p className="text-slate-600 text-xs">Gestiona accesos, roles y permisos de tu equipo</p>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar-light-light-light">
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {usuarios.map(u => (
                <div key={u.id} className={`bg-white border rounded-2xl p-5 flex flex-col justify-between shadow-sm hover:shadow-md hover:border-slate-400 transition-all duration-150 ${
                  u.activo === false ? 'opacity-60 border-dashed border-red-300 bg-red-50/5' : 'border-slate-300'
                }`}>
                  <div>
                      ...
                      {u.role === 'admin' ? (
                        ...
                        <div className="flex flex-wrap gap-1">
                          {u.permisos.map(p => {
                            ...
                            return (
                              <span key={p} className="text-[9px] text-slate-700 bg-slate-100 border border-slate-300 px-1.5 py-0.5 rounded" title={label}>
                                {p.split(':')[1]}
                              </span>
                            );
                          })}
                  ...
                  <div className="flex items-center gap-2 pt-3 border-t border-slate-300/50 mt-2">
                    <button onClick={() => editUser(u)} className="flex-1 bg-slate-100 hover:bg-slate-750 text-slate-800 rounded-lg py-2 flex items-center justify-center gap-2 text-sm transition-colors border border-slate-300">
      ...
      <div className="w-[400px] min-w-[350px] max-w-[450px] bg-white border border-slate-200 rounded-2xl p-5 flex flex-col shadow-2xl overflow-y-auto custom-scrollbar-light-light-light">
        ...
        {mensaje.texto && (
          <div className={`mb-4 p-3 rounded-xl text-sm font-medium flex items-center gap-2 ${
            mensaje.tipo === 'error' ? 'bg-red-500/10 text-red-400 border border-red-500/20' : 'bg-emerald-500/10 text-emerald-600 border border-emerald-500/20'
          }`}>
            <AlertCircle size={16} />
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col gap-4">
            ...
            <div>
              ...
              <div className="relative">
                <input 
                  ...
                  className="w-full bg-white border border-slate-350 hover:border-slate-400 rounded-xl p-2.5 pr-10 text-sm text-slate-900 focus:border-blue-500 outline-none transition-colors shadow-sm" 
                  placeholder="••••••••"
                />
                <button 
                  type="button" 
                  onClick={() => setVerContra(!verContra)} 
                  className="absolute right-3 top-3.5 text-slate-500 hover:text-slate-600"
                >
                  {verContra ? <EyeOff size={16} /> : <Eye size={16} />}
                </button>
              </div>
              <p className="text-[10px] text-slate-500 mt-1">Mínimo 6 caracteres</p>
            </div>
          </div>

          {/* Grupo 2: Roles y Estado */}
          <div className="bg-slate-50 border border-slate-300 rounded-2xl p-4 space-y-3.5 shadow-sm">
            <span className="block text-[10px] font-extrabold text-slate-700 uppercase tracking-wider mb-1">Roles y Estado</span>
            <div>
              <label className="block text-[10px] font-bold text-slate-600 uppercase tracking-wider mb-1">Nivel de Acceso (Rol) *</label>
              <select 
                value={form.role} 
                onChange={e => setForm({...form, role: e.target.value as 'admin'|'colaborador'})} 
                className="w-full bg-white border border-slate-350 hover:border-slate-400 rounded-xl p-3 text-slate-900 focus:border-blue-500 outline-none transition shadow-sm cursor-pointer text-sm"
              >
            ...
            <div className="flex items-center justify-between p-3 bg-white border border-slate-200 rounded-xl shadow-sm">
              <div>
                <span className="text-slate-750 text-xs font-semibold block">Estado del Usuario</span>
                <span className="text-[9px] text-slate-550">{form.activo ? 'Permitido iniciar sesión' : 'Acceso bloqueado'}</span>
              </div>
              <label className="relative inline-flex items-center cursor-pointer">
                ...
                />
                <div className="w-11 h-6 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-slate-400 after:border-slate-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-emerald-500 peer-checked:after:bg-slate-50"></div>
              </label>
            </div>
          </div>
          ...
          {form.role === 'colaborador' && (
            <div className="border border-slate-300 bg-slate-50/60 rounded-xl p-4 mt-2 shadow-sm">
              <span className="text-slate-650 text-xs font-semibold uppercase tracking-wider block mb-3">Permisos de Acceso</span>
              
              <div className="space-y-3">
                {PERMISOS_DISPONIBLES.map(p => {
                  const hasPerm = form.permisos.includes(p.id);
                  return (
                    <label key={p.id} className="flex items-center gap-3 cursor-pointer select-none text-slate-700 hover:text-slate-900 transition">
                      <input 
                        type="checkbox" 
                        checked={hasPerm} 
                        onChange={e => handlePermissionChange(p.id, e.target.checked)} 
                        className="w-4.5 h-4.5 accent-blue-500 rounded border-slate-300 bg-white"
                      />
          ...
          <div className="mt-auto pt-6 flex flex-col gap-3">
            <button 
              disabled={!canSubmit}
              type="submit" 
              className={`w-full font-bold py-3.5 rounded-xl shadow-lg transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer ${
                successSaved 
                  ? 'bg-emerald-600 text-white shadow-emerald-600/20'
                  : !canSubmit 
                    ? 'bg-slate-100 text-slate-400 cursor-not-allowed shadow-none border border-slate-250'
                    : 'bg-blue-600 hover:bg-blue-500 text-white shadow-blue-500/20 hover:scale-[1.01] active:scale-[0.99]'
              }`}
            >
              {successSaved ? (
                <>
            ...
            {editMode && (
              <button type="button" onClick={resetForm} className="w-full bg-slate-200 hover:bg-slate-300 text-slate-800 font-bold py-3 rounded-xl transition-all border border-slate-350 cursor-pointer shadow-sm">
                Cancelar
              </button>
            )}
          </div>
